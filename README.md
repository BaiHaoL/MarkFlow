...
- 语法高亮提示（代码块、标题、列表等，覆盖 11 种 Markdown 元素）；高亮分层：≤64K 字符同步即时计算，>64K 字符后台线程 + 80ms 防抖异步计算（输入不卡顿）；>32K 或富结构（段落数超预算）的文档编辑态自动降级为仅标题高亮，避免长篇高亮卡顿；围栏代码块内部屏蔽所有高亮规则
...
- **插入图片**：复制到 `.md` 同级 `images/` 目录（压缩长边 2048px、JPEG 质量 85、命名 `mf_{时间戳}_{随机4位}.jpg`——`mf_` 前缀用于区分自动副本与用户自命名图片，防误删）；写入后自动放置 `.nomedia` 并触发媒体重扫，**复制来的图片不会出现在系统相册**；删除 `.md` 时自动清理该目录下不再被任何现存 `.md` 引用的自动命名图片副本（用户自放的图片绝不误删）
- 编辑工具栏图标顺序：**搜索 → 撤销 → 反撤销 → 插入图片**（均为编辑模式专属）
- 分段编辑兼容手写输入法（保留输入法组合区间，多笔正常成字）
- 键盘避让：编辑内容区恒满高，键盘高度**实时感知**（含拼音⇄手写等高度切换），弹出时通过光标跟随将光标抬至键盘上方，既避免内容被键盘遮挡，也不会因压缩内容区高度导致正文空白；滚动内容底部自带留白，末行不会贴屏幕底
